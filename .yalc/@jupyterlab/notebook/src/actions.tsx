...
namespace Private {
  ...
  async function runCell(
    ...
    // Ensure cell and cell.model are valid
    if (!cell || !cell.model) {
    ...
    if (cell.model.type === 'code') {
      ...
      if (metadata) {
        const isRunning = metadata['running'] as boolean | undefined;
        if (isRunning) {
          console.log(`Cell ${cell.model.id ?? 'unknown'} is already running, ignoring execution request`);
          return Promise.resolve(false);
        }
        // Mark the cell as running
        metadata['running'] = true;
      } else {
        console.warn(`Cell ${cell.model.id ?? 'unknown'} has no metadata, cannot set running state`);
    ...
    try {
      ...
      return await (executor ? executor.runCell(options) : defaultRunCell(options));
    } finally {
      // Clear the running state for code cells if metadata exists
      if (cell.model?.type === 'code' && cell.model.metadata) {
        delete (cell.model.metadata as Record<string, any>)['running'];
      }
    }
  }
