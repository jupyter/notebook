...
define(
  ...
  function($, QH, dialog, marked, React, ReactDOM) {
    /**
    ...
    class KeyBinding extends React.Component {
      constructor(props) {
        super(props);
        this.handleShrtChange = this.handleShrtChange.bind(this);
        this.state = {
          shrt: ""
        };
      }

      handleShrtChange(event) {
        this.setState({ shrt: event.target.value });
      }
      ...
      render() {
        ...
        var binding_setter = () => {
          if (available) {
            this.props.onAddBindings(this.state.shrt, this.props.ckey);
          }
          this.setState({ shrt: "" });
          return false;
        };
        return React.createElement(
          ...
          React.createElement("input", {
            ...
            className:
              "pull-right" + (available || empty ? "" : " alert alert-danger"),
    ...
    class KeyBindingList extends React.Component {
      constructor(props) {
        super(props);
        this.state = {
          data: []
      ...
      render() {
        var children = this.state.data.map(binding => {
          return React.createElement(
            KeyBinding,
            Object.assign({}, binding, {
              onAddBindings: (shortcut, action) => {
    ...
    var get_shortcuts_data = function(notebook) {
      ...
      for (var i = 0; i < actions.length; i++) {
        ...
        var shortcuts = notebook.keyboard_manager.command_shortcuts.get_action_shortcuts(
          action_id
        );
        var hshortcuts = [];
    ...
    var ShortcutEditor = function(notebook) {
      ...
      ReactDOM.render(
        React.createElement(KeyBindingList, {
          callback: function() {
            return get_shortcuts_data(notebook);
          },
          bind: function(shortcut, command) {
            return notebook.keyboard_manager.command_shortcuts._persist_shortcut(
              shortcut,
              command
            );
          },
          unbind: function(shortcut) {
            return notebook.keyboard_manager.command_shortcuts._persist_remove_shortcut(
              shortcut
            );
          },
          available: function(shrt) {
            return notebook.keyboard_manager.command_shortcuts.is_available_shortcut(
              shrt
            );
          }
        }),
